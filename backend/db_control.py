...
class db:
    ...
    def drivers(self, meeting_key, driver_number):
        # lap informations
        response = urlopen(
            f"https://api.openf1.org/v1/laps?meeting_key={meeting_key}&driver_number={driver_number}"
        )
    ...
    def laps(self, meeting_key):

        # lap informations
        response = urlopen(f"https://api.openf1.org/v1/laps?meeting_key={meeting_key}")
